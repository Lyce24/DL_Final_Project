...
import pandas as pd
import os
from models.models import ViTIngrModel, CLIngrModel
...
def test_model_ingr(model, dataloader, device, log_min_max):
    ...
    with torch.no_grad():
        for inputs, targets in dataloader:
            ...
            regression_loss = ingredient_loss(outputs[1], targets[1], targets[0], model_params=model.parameters(), l2_lambda = 0.01)
...
def eval(model_backbone, save_name, test_loader, log_min_max, s, device):
    ...
    print(f"Regression Loss (WMSE + l2): {overall_regression_loss}\nClassification Loss: {overall_classifcation_loss}\nClassification Accuracy: {classification_accuracy}\nPrecision: {precision}\nRecall: {recall}\nF1-Score: {f1_score}")
    ...
    if s:
        ...
        with open(save_path, 'w') as f:
            ...
            f.write(f"Regression Loss (WMSE + l2): {overall_regression_loss}\nClassification Loss: {overall_classifcation_loss}\nClassification Accuracy: {classification_accuracy}\nPrecision: {precision}\nRecall: {recall}\nF1-Score: {f1_score}\n")
            f.write('\n')
