...
class ResNetIngrModel(nn.Module):
    def __init__(self, num_ingr: int = 199, pretrained = False, hidden_dim_1: int = 1024, hidden_dim_2: int = 512, dropout_rate: float = 0.3):
        """
        Args:
        ...
        # Custom fully connected layers for regression
        self.fc_layers = nn.Sequential(
            nn.Linear(2048, hidden_dim_1), # 2048 is the output size of ResNet50
        ...
        # ReLU activation for non-negative mass prediction
...
if __name__ == "__main__":
    ...
    model = ResNetIngrModel(199)
    output = model(x)
    print(f"ResNet Ingr model output shape: {output.shape}") # Expected output shape: [16, 199]
